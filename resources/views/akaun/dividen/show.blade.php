...
<div class="page-head">
    <div>
        ...
        <p class="lead">Cut-off {{ $dividen->tarikh_cutoff->translatedFormat('d M Y') }} · {{ $dividen->shares->count() ?? $shares->total() }} ahli</p>
    </div>
...
<div class="grid grid-3" style="margin-bottom:24px;">
    <div class="stat"><div class="k">Untung Bersih</div><div class="v" style="font-size:24px;">RM {{ number_format($dividen->untung_bersih, 2) }}</div></div>
    ...
    <div class="stat" style="border-color:var(--gold-soft);"><div class="k">Jumlah Dividen ({{ rtrim(rtrim(number_format($dividen->peratus_dividen,2),'0'),'.') }}%)</div><div class="v" style="font-size:24px;color:var(--gold);">RM {{ number_format($dividen->jumlah_dividen, 2) }}</div></div>
</div>
...
<div class="grid grid-2" style="margin-bottom:24px;align-items:start;">
    ...
    <div class="panel">
        ...
        <div class="panel-body">
            @if ($draf)
                <form method="POST" action="{{ route('akaun.dividen.update', $dividen) }}">
                    ...
                    <div class="grid grid-2">
                        ...
                        <div class="field">
                            <label>Peratus Dividen (%)</label>
                            <input class="input" type="number" name="peratus_dividen" step="0.01" min="0" max="100" value="{{ $dividen->peratus_dividen }}" required>
                        </div>
                    </div>
            ...
            @else
                <div class="field"><label>Tarikh Cut-off</label><div>{{ $dividen->tarikh_cutoff->translatedFormat('d M Y') }}</div></div>
